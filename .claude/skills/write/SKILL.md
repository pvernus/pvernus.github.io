...
**Agent:** Writer
**Output:** Section written into `output/paper/draft.qmd` (source of truth for this project)

**Quarto note:** This project uses Quarto (`.qmd`), not LaTeX. When editing `draft.qmd`:
- Use `#|` chunk options (not `opts_chunk`): `#| echo: false`, `#| fig-cap: "..."`
- Cross-references use `@fig-label`, `@tbl-label`, `@sec-label` — not `\ref{}`
- Citations use `@citekey` — not `\cite{}`
- For full Quarto syntax reference, read: `claude_lib/posit-skills-main/quarto/quarto-authoring/references/`
  - `code-cells.md` — chunk options
  - `cross-references.md` — figure/table/section refs
  - `citations.md` — bibliography syntax
  - `figures.md` — figure layout
  - `tables.md` — table syntax

Workflow:
1. Read existing paper, research spec, literature review, results summary
2. Read domain-profile.md for field conventions
3. Check `output/paper/references.bib` for available citations
